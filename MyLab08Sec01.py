...
def dice(intNumber, centerPoint, intSize, graphwin):
    ...
    for d, p in zip(dots, points[intNumber - 1]):
        # zip(): [1, 2, 3, 4] & [a, b, c] -> [(1, a), (2, b), (3, c)] (omit the excess)
...
def inArea(rectangle, point):
    ...
    return leftEnd < pointX < rightEnd and topEnd < pointY < bottomEnd # return Boolean
...
def main():
    ...
    while True:
        ...
        if inArea(textboxClick[1], clicked):
            ...
            if 1 <= intNumber <= 6:
                dice1 = dice(intNumber, Point(intWidth // 2, intHeight // 2), intSize, win)
            else:
